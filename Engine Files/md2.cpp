...
void CMD2Model::SetAnimation(int state, int nextState) const
{
  switch (state)
  {
  case RUN:
...
void CMD2Model::SetAnimationCustom(int start, int end) const
{
  if (start < 0)
    start = 0;
...
void CMD2Model::Animate(float seconds) const
{
...
void CMD2Model::Render() const
{
  glTranslatef(m_pos.x, m_pos.y, m_pos.z);
  glRotatef(m_rotate+270.0f, 0, 1, 0);
