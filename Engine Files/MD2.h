...
public:
  ...
  void SetAnimationCustom(int startFrame, int endFrame) const;
  void SetAnimation(int state, int nextState = _REPEAT) const;
  void Animate(float seconds) const;
...
private:
  mutable int m_state;        // current model animation state
  mutable int m_nextState;    // state to automatically transition to after the current state is finished
  mutable int m_currentFrame; // current frame # in animation
  mutable int m_nextFrame;    // next frame # in animation
  mutable int m_startFrame;   // first frame in current animation
  mutable int m_endFrame;     // last frame in current animation

  sphere_t m_boundingSphere;
  mutable float m_interpol;          // percent through current frame
